...
int send_renew(unsigned long xid, unsigned long server, unsigned long ciaddr)
{
	...
	add_requests(&packet);
	LOG(LOG_DEBUG, "Sending renew... (%08x) %s", server, server?"(unicast)":"(broadcast)");
	if (server) 
#ifdef USE_DUAL_WAN
	{
		LOG(LOG_DEBUG, "Sending renew... raw packet (%08x) %s", server, server?"(unicast)":"(broadcast)");
		...
		ret = kernel_packet(&packet, ciaddr, CLIENT_PORT, server, SERVER_PORT);
				...
				SERVER_PORT, MAC_BCAST_ADDR, client_config.ifindex);
#else
	/* Windows XP will generate Broadcast request which have source ip of ciaddr , not zero */  
	/* scchoi 2006-06-08 */
...
int get_raw_packet(struct dhcpMessage *payload, int fd)
{
	...
	if (bytes < 0) {
		LOG(LOG_DEBUG, "couldn't read on raw listening socket -- ignoring");
		usleep(500000); /* possible down interface, looping condition */
		return -1;
	}
	
	if (bytes < (int) (sizeof(struct iphdr) + sizeof(struct udphdr))) {
		LOG(LOG_DEBUG, "message too short, ignoring");
		return -2;
	}
	
	if (bytes < ntohs(packet.ip.tot_len)) {
		LOG(LOG_DEBUG, "Truncated packet");
		return -2;
	}
	...
	if (packet.ip.protocol != IPPROTO_UDP || packet.ip.version != IPVERSION ||
	    ...
	    ntohs(packet.udp.len) != (short) (bytes - sizeof(packet.ip))) {
	    	//LOG(LOG_DEBUG, "unrelated/bogus packet: %d", packet.udp.dest );
	    	return -2;
	}
	...
	if (check != checksum(&(packet.ip), sizeof(packet.ip))) {
		LOG(LOG_DEBUG, "bad IP header checksum, ignoring");
		return -1;
	}
	...
	if (check && check != checksum(&packet, bytes)) {
		LOG(LOG_DEBUG, "packet with bad UDP checksum received, ignoring");
		return -2;
	}
