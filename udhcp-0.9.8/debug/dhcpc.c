...
#endif
{
	...
	{
		printf("Start USB Tethering DHCLIENT\n");
		usb_tethering=1;
	}
#endif
#ifdef USE_DHCP_AUTO_DETECT
	...
	{
		printf("Start DHCLIENT for Checking\n");
		dhclient_chk=1;
	}
#endif
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (packet_num < 3) {
					...
					{
#ifdef USE_SYSTEM_LOG
						if(get_dhcp_auto_detect_status()) syslog_msg( SYSMSG_LOG_INFO, SYSLOG_MSG_DHCP_SERVER_RESUME );
#endif
						set_dhcp_auto_detect_status(0);
						exit_client(0);
					}
#endif
					...
					if(!syslog_get_flag(client_config.interface))
					{
						...
						syslog_msg( SYSMSG_LOG_INFO, SYSLOG_MSG_DHCPC_SERVER_TIMEOUT" ( %s )", get_wan_name(client_config.interface,1));
					}
...
#ifdef USE_SYSTEM_LOG
					syslog_msg( SYSMSG_LOG_INFO, SYSLOG_MSG_DHCPC_LEASE"( %s )", get_wan_name(client_config.interface, 1));
#endif
...
#ifdef USE_SYSTEM_LOG
					if(!get_dhcp_auto_detect_status())
						syslog_msg( SYSMSG_LOG_INFO, SYSLOG_MSG_DHCP_SERVER_SUSPENDED );
#endif

					set_dhcp_auto_detect_status(1);
				}
				//else
				//	set_dhcp_auto_detect_status(0);
				exit_client(0);
