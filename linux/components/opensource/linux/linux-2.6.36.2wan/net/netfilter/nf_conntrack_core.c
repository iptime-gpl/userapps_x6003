                      ...
                      struct nf_conntrack_tuple *manip)
{
	...
	if ((skb == NULL) || (ct == NULL))
        ...
        if (check_internal_subnet(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip) &&
            check_internal_subnet(ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3.ip))
        {
                #if 0
                printk("===================================================\n");
                printk("ORG: ");NF_CT_DUMP_TUPLE(&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple);
                printk("RPY: ");NF_CT_DUMP_TUPLE(&ct->tuplehash[IP_CT_DIR_REPLY].tuple);
                #endif
                return;
        }
...
#if defined CONFIG_NETFILTER_XT_TARGET_RTMARK || defined CONFIG_NETFILTER_XT_TARGET_RTMARK_MODULE
        if (skb->dev->name)
        {
#if 0 // ysyoo
		struct iphdr *iph = ip_hdr(skb);
                if ((iph->protocol == IPPROTO_TCP) && strcmp(skb->dev->name, CONFIG_LOCAL_IFNAME))
                {
                        printk("dev:%s, ctrakmark:%x, saddr:%08x daddr:%08x\n",
                                skb->dev->name, ct->rtmark, iph->saddr, iph->daddr);
                }
#endif
                if (!strcmp(skb->dev->name, CONFIG_LOCAL_IFNAME))
                {
			//printk("Local_IF ctrtmark:%x, skbmark:%d\n", ct->rtmark, skb->mark);
                        if (ct->rtmark == WAN1_INPUT_RTMARK)
                                skb->mark = WAN1_RTID_RTMARK;
                        else if (ct->rtmark == WAN2_INPUT_RTMARK)
                                skb->mark = WAN2_RTID_RTMARK;
                        else
                                skb->mark = ct->rtmark;
                }
                else if (!strcmp(skb->dev->name, "vlan2") || !strcmp(skb->dev->name, "ppp1"))
                        skb->mark = WAN1_INPUT_RTMARK;
                else if (!strcmp(skb->dev->name, "vlan3") || !strcmp(skb->dev->name, "ppp2"))
                        skb->mark = WAN2_INPUT_RTMARK;
                else if (!strncmp(skb->dev->name, "ppp", 3) && // pptp vpn client loadshare
                        ((ct->rtmark == WAN1_RTID_RTMARK) || (ct->rtmark == WAN2_RTID_RTMARK)))
                        skb->mark = ct->rtmark;
                else
                        skb->mark = MAIN_RTID_RTMARK;
        }
#endif
...
#ifdef CONFIG_CONNTRACK_STATISTICS_RATE
		if( (jiffies - ct->tuplehash[NF_CT_DIRECTION(h)].tstamp) > CONFIG_CONN_TSTAMP_PERIOD)
		{
			ct->tuplehash[NF_CT_DIRECTION(h)].prev_bytes = ct->tuplehash[NF_CT_DIRECTION(h)].bytes;
			ct->tuplehash[NF_CT_DIRECTION(h)].tstamp = jiffies;
		}
#endif
        }
#endif
