...
static bool binary_search(struct dstmatch_pool *dpool, unsigned int target)
{
...
{
	const struct xt_dstmatch_info *info = par->matchinfo;
...
//	printk("--> dstmatch : %s \n", info->name);
	...
	for (n = 0; n < MAX_POOL; n++)
	{
		if (!strcmp(dmentry[n].name, info->name))
			return binary_search(&dmentry[n], ntohl(iph->daddr));
	}
...
static int checkentry(const struct xt_mtchk_param *par)
{
	...
	if (info->name[0] == '\0')
	{
		printk("'dstmatch' match check error. name is NULL\n");
		return -EINVAL;
	}
	...
	for (n = 0; n < MAX_POOL; n++)
	{
		...
		for (k=0; k<dmentry[n].ecount; k++, entry++)
			p += sprintf(p,"%u.%u.%u.%u ~ %u.%u.%u.%u\n", 
				((unsigned char *)&entry->sip)[0],
				((unsigned char *)&entry->sip)[1],
				((unsigned char *)&entry->sip)[2],
				((unsigned char *)&entry->sip)[3],
				((unsigned char *)&entry->eip)[0],
				((unsigned char *)&entry->eip)[1],
				((unsigned char *)&entry->eip)[2],
				((unsigned char *)&entry->eip)[3]);
	}
...
static struct xt_match dstmatch_match
= { 
	.name = "dstmatch", 
	...
	.match = match,
	.destroy  = destroy,
	.matchsize  = sizeof(struct xt_dstmatch_info),
	.me = THIS_MODULE
};
...
static int __init init(void)
{
	...
	proc_entry = create_proc_entry("xt_dstmatch", 0, 0);
	if( proc_entry != NULL )
	{
		proc_entry->read_proc = &proc_read;
	}

	return xt_register_match(&dstmatch_match);
}

static void __exit fini(void)
{
	xt_unregister_match(&dstmatch_match);
}
