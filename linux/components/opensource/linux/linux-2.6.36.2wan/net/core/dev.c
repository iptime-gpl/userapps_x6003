...
int BCMFASTPATH_HOST dev_queue_xmit(struct sk_buff *skb)
{
	...
	if (q->enqueue
            ...
            && (skb->mark || !strncmp(dev->name, "wl", 2)) // mssid interface
#endif
	) {
...
#define WL_5G_PREFIX "wl0"
#define WL_2G_PREFIX "wl1"
#endif
...
#ifdef CONFIG_QOS_INTERFACE_MARK
        if (rcu_dereference(skb->dev->qdisc) && !strncmp(skb->dev->name, "wl", 2))
        {
                int index = 0, base=0;

                if (!strncmp(skb->dev->name, WL_2G_PREFIX, 3))
		{
                        sscanf(skb->dev->name,WL_2G_PREFIX".%d", &index);
			base = INTERFACE_MARK_SUBID_BASE;
		}
                else if (!strncmp(skb->dev->name, WL_5G_PREFIX, 3))
		{
                        sscanf(skb->dev->name,WL_5G_PREFIX".%d", &index);
